...
import { villages } from "@/lib/grambiz-data";
  ...
  head: () => ({
    meta: [
      { title: "Sign in or sign up — GramBiz AI" },
      {
        name: "description",
        content:
          "Create your free GramBiz AI account to save your village, budget and business plans in Hindi or English.",
      },
...
function AuthPage() {
  ...
  const [village, setVillage] = useState(villages[0]!.name.en);
      ...
      if (mode === "signup") {
        const { error: err } = await supabase.auth.signUp({
          ...
          options: {
            ...
            data: { full_name: fullName, village, phone },
          },
        });
        if (err) throw err;
      } else {
    ...
    <div className="flex flex-col gap-3">
      <section className="panel-dark rounded-2xl p-4">
        ...
        <p className="mt-0.5 text-xs text-paper/60">
          {hi
            ? "अपना गाँव और बजट सेव रखने के लिए खाता बनाइए"
            : "Save your village, budget and business ideas"}
        </p>
          ...
          {mode === "signup" && (
            <>
              ...
              <label className="flex items-center gap-2 rounded-xl bg-paper/95 px-3 py-2.5">
                ...
                >
                  {villages.map((v) => (
                    <option key={v.id} value={v.name.en}>
